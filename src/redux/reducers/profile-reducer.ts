...
export const getUserStatus =
  (userId: number): ThunkType =>
  (dispatch) => {
    return profileAPI.getUserStatus(userId).then(({ data }) => dispatch(actions.setStatus(data)));
  };
...
export const getFollowInfo =
  (userId: number): ThunkType =>
  (dispatch) => {
    return profileAPI.getFollowInfo(userId).then(({ data }) => dispatch(actions.setFollowInfo(data)));
  };
